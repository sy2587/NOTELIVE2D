...
<template>
  ...
  <main id="main-content">
    <section class="hero" aria-labelledby="hero-title">
      <div class="hero-copy"><p class="eyebrow"><span></span>Study with intention</p><h1 id="hero-title">把複雜的學習，<em>整理成自己的節奏。</em></h1><p class="hero-intro">一個安靜、清楚的學習空間。把筆記、科目和每日任務放在一起，讓注意力回到真正重要的事。</p><div class="hero-actions"><RouterLink class="button button-primary" :to="auth.isAuthenticated ? '/' : '/register'">{{ auth.isAuthenticated ? '回到我的書桌' : '建立我的學習空間' }}<span aria-hidden="true">→</span></RouterLink><a class="text-link" href="#features">先看看怎麼使用</a></div><div class="trust-row" aria-label="產品特色"><span>不用信用卡</span><span>隨時可以開始</span><span>專注、沒有廣告</span></div></div>
